...
# ─── Docker socket: синхронизация GID ─────────────────────────────────────
# Если docker.sock примонтирован, подгоняем GID группы docker в контейнере
# под GID сокета на хосте, чтобы пользователь claude мог им пользоваться
if [[ -S /var/run/docker.sock ]]; then
    host_docker_gid=$(stat -c '%g' /var/run/docker.sock 2>/dev/null || echo "")
    if [[ -n "$host_docker_gid" && "$host_docker_gid" != "0" ]]; then
        # Меняем GID группы docker (или создаём её)
        if getent group docker &>/dev/null; then
            groupmod -g "$host_docker_gid" docker 2>/dev/null || true
        else
            groupadd -g "$host_docker_gid" docker 2>/dev/null || true
        fi
        usermod -aG docker "$CLAUDE_USER" 2>/dev/null || true
        success "Docker socket доступен (GID=$host_docker_gid)"
    elif [[ "$host_docker_gid" == "0" ]]; then
        # Сокет принадлежит root — добавляем разрешение через chmod
        chmod 666 /var/run/docker.sock 2>/dev/null || true
        success "Docker socket доступен (root-owned, chmod 666)"
    fi
else
    info "Docker socket не примонтирован — docker-команды будут недоступны"
fi
